...
lib.genAttrs systems.default (
  ...
  then { }
    ...
    in
    {
      ...
      host-architecture =
        # Note: thinsandy assertions removed — host decommissioned in
        # commit "frieren: complete thinsandy decommission" and no longer
        # in flake/host-inventory.nix. The hermes-agent assertions are
        # kept commented as historical reference only.
