...
- `taskfiles/dev.yml` contains git/flake/local helper workflows (now including `dev:flake:update:bountystash`).
...
- `taskfiles/dev.yml` – git/flake helpers (flake updates, site sync).
...
- Flake/git helpers and `bountystash` updates → `taskfiles/dev.yml`.
...
- Operator helpers and manifest lookups → `taskfiles/agents.yml` and `scripts/task/ai-host-manifest.sh`.
